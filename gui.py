...
class ModellingTab(Color):
    def __init__(self, parent, *args, **kwargs):
        ...
        column2layout.addWidget(QLabel("Orders"))
        self.orders_list = QListWidget()
        self.orders_list.addItems(['A', 'C', 'D', 'C'])
        column2layout.addWidget(self.orders_list)

        self.executed_orders_list = QListWidget()
        column2layout.addWidget(self.executed_orders_list)

        column2.addLayout(column2layout)
    ...
    def book_info(self, obj):
        ...
        desc.append(self.parent.book_qty[row]) # qty
        desc.append(book.genre)
        desc.append(self.parent.store.get_real_price(book))
    ...
    def update_executed_orders(self, exec_orders):
        #self.executed_orders_list.clear()
        self.executed_orders_list.addItems(exec_orders)
...
class newWindow(QMainWindow):
    ...
    def settings_set(self):
        ...
        self.page_1.executed_orders_list.clear()

    def update_books(self, books_w_qty):
        self.books, self.book_qty = zip(*books_w_qty)
        books = [str(b)+" : "+str(q) for b, q in books_w_qty]
    ...
    def update_executed_orders(self, exec_orders):
        self.page_1.update_executed_orders(exec_orders)
